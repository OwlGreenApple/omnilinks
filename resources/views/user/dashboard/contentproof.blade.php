  ...
  @foreach($query as $proof)
    <div class="card card-none mb-3">
      <div class="card-header card-gray">
        <span class="view-wa" data-toggle="collapse" data-target="#detail-{{$proof->id}}" aria-expanded="true" aria-controls="detail-{{$proof->id}}">
          {{$proof->name}}
          <span class="ml-2">
        ...
        <button type="button" dataid="{{$proof->id}}" class="btn btn-sm btn-danger float-right btn-delete">
          <i class="fas fa-trash-alt"></i>
        </button>

        <button type="button" class="btn btn-sm btn-primary float-right btn-editpixel" dataedit="{{ $proof->id }}" style="margin-right:5px; ">
